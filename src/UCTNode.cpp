    ...
    **/
    ...
    while ((moves_searched < random_search_count) && (randomX_100 <= 5) && !is_opponent_move) { // Wide search loop for Tiebot's turn.
        for (auto& child : m_children) {
            ...
            if (!is_opponent_move && (winrate >= 0.01) && (winrate >= (1.10 * winrate_target_value))) {
                continue;
            }
    ...
    while ((moves_searched < random_search_count) && (is_pondering_now == true) && (1 == 0)) { // Wide search loop for pondering on opponent's turn.
        for (auto& child : m_children) {
            if (!child.active()) {
                continue;
            }
