...
void UCTNode::prepare_root_node(Network & network, int color, // redundant argument?
	...
	///////////////// THE FOLLOWING SEVEN LINES ARE MISSING IN THE DYN-KOMI CODE. I DON'T KNOW WHY. ALSO, ONE LINE CHANGED WHEN I MERGED AND IT SEEMS UNRELATED TO DYN-KOMI AND I DON'T KNOW WHY IT CHANGED.
    if (had_children) {
        root_eval = get_net_eval(color); ////////////// THIS LINE CHANGED. ORIGINAL WAS:             root_eval = get_eval(color);
    } else {
        update(root_eval);
